...
function TreeNode(text, url, iconOpen, iconOpenHover, iconClosed,
		iconClosedHover) {
	var $ = this;
	this.level = 0;
	this.children = [];
	...
	this.PATH = "../images/skin_blue/tree/";
	...
	this.CATEGORYCLOSED = this.PATH
			+ (iconClosed ? iconClosed : "folder_closed.gif");
	...
	this.label.style.fontFamily = "微软雅黑";
	...
	if (url) {
		...
		this.label.href = "javascript:self.parent.changeFrame('" + url + "');";
	...
	this.setIcon = function() {
		this.nodeIcon = this.status == "CLOSED" ? ($.isHover ? $.CATEGORYCLOSED_HOVER
				: $.CATEGORYCLOSED)
	...
	this.node.onmousedown = function() {
		if (currentNode) {
			currentNode.nodeIcon.src = (currentNode.status == "CLOSED" ? currentNode.CATEGORYCLOSED
					: currentNode.CATEGORYOPEN);
		}
	}
