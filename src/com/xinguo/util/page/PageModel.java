...
public class PageModel {
	...
	public  int getMaxItme() {
    	...
    	try {
    		...
    		config.load(PageModel.class.getResourceAsStream("/config/config.properties"));
    		logger.debug("config :"+config);
    		logger.debug("config :"+config.getProperty("some.maxItme"));
    	} catch (Exception e) {
        	logger.debug("load config file is filed,it will been default"+e.getMessage());
        }
    	if(config !=null && config.getProperty("some.maxItme") != null && config.getProperty("some.maxItme").matches("\\d+"))
    		maxItme = new Integer(config.getProperty("some.maxItme"));
    	logger.debug("max itme init is "+maxItme);
